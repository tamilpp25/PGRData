...
local InitCampIdList = function()
    for id in pairs(CampTemplate) do
        tableInsert(CampIdList, id)
    end
end
...
function XGuardCampConfig.GetCampIdList()
    return CampIdList
end
...
function XGuardCampConfig.GetActivityId()
    ...
    for id in pairs(ActivityTemplate) do
        endTimestamp = XGuardCampConfig.GetActivityTime(id)
