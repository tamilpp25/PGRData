...
function XRobot:UpdateAbility()
    ...
    --伙伴战力
    local partnerAbility = not XTool.IsTableEmpty(self.Partner) and XDataCenter.PartnerManager.GetCarryPartnerAbility(self.Partner) or 0
...
function XRobot:GenarateRobotEquipResonanceInfo(pos)
    ...
    for slot, templateId in pairs(templateIdList) do
        ...
        if not XTool.IsNumberValid(templateId)
        or not XTool.IsNumberValid(resonanceType)
        then
            local path = XRobotManager.GetConfigPath()
            XLog.Error("XRobot:GenarateRobotEquipResonanceInfo error: 机器人共鸣类型（WaferResonanceType）与共鸣技能Id（WaferResonance）不匹配, robotId: " .. robotId .. ", path: " .. path)
            break
        end
...
function XRobot:GenarateRobotSkillList()
    ...
    for _, skillGroup in pairs(skillDic) do
        for _, skillGroupId in pairs(skillGroup) do
            ...
            if XTool.IsNumberValid(skillId)
            and not removeDic[skillId] then
                local skillInfo = {
                    Id = skillId,
