...
function XServerManager.InsertTempServer(ip)
    ...
    local tempServer = {
        ...
        Name = "临时服: " .. ip,
        LastTime = 0,
        LoginUrl = string.format("http://%s:2333/api/Login/Login", ip),
        IsTempServer = true,
    }

    table.insert(ServerList, tempServer)
    TempServerDic[ip] = true
    XServerManager.UpdateSortedServer()
...
function XServerManager.Select(server)
    ...
    elseif true or Platform == RuntimePlatform.IPhonePlayer then
