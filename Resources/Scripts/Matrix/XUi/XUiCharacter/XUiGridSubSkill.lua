...
XUiGridSubSkill = XClass(nil, "XUiGridSubSkill")
...
function XUiGridSubSkill:RegisterListener(uiNode, eventName, func)
    ...
    if func ~= nil then
        if type(func) ~= "function" then
            XLog.Error("XUiGridSubSkill:RegisterListener函数错误, 参数func需要是function类型, func的类型是" .. type(func))
        end
...
function XUiGridSubSkill:UpdateGrid(characterId, subSkillInfo)
    ...
    local addLevel = XDataCenter.CharacterManager.GetSkillPlusLevel(characterId, subSkillInfo.SubSkillId)
