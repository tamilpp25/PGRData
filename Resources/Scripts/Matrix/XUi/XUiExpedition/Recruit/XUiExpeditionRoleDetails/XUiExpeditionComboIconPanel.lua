...
function XUiExpeditionComboIconPanel:UpdateData(eChara)
    if self.DetailsType == DetailsType.FireMember then
        self.ComboList = XDataCenter.ExpeditionManager.GetComboList():GetActiveComboIdsByEChara(eChara)
    elseif self.DetailsType == DetailsType.RecruitMember then
        self.ComboList = XDataCenter.ExpeditionManager.GetComboList():GetPreviewCombosWhenRecruit(eChara)
    end
