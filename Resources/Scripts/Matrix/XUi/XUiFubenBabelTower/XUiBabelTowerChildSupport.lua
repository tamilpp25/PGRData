...
function XUiBabelTowerChildSupport:OnBtnGoClick()
    local babelTowerData = {
        TeamId = self.TeamId,
        TeamList = XDataCenter.FubenBabelTowerManager.GetCacheTeam(self.StageId, self.TeamId, self.TeamList, self.CaptainPos, self.FirstFightPos),
        Cb = function(resTeam, captainPos, firstFightPos)
            self.TeamList = resTeam
            self.CaptainPos = captainPos
            self.FirstFightPos = firstFightPos
        end
    }
    local data = {BabelTowerData = babelTowerData}
