...
function XUiGuildBossCurSubLevelInfo:RealOnBtnStartClick()
    ...
    if self.DetailData.BuffLeft >= GUILD_BOSS_NEED_BUFF then
        CsXUiManager.Instance:Open("UiDialog", self.EnterWarningTitleStr, CS.XTextManager.GetText("GuildBossBuffActive"), XUiManager.DialogType.Normal, nil, function()
    ...
    elseif self.Data.UploadCount == 0 then
        ...
        CsXUiManager.Instance:Open("UiDialog", self.EnterWarningTitleStr, context, XUiManager.DialogType.Normal, nil, function()
            XLuaUiManager.Open("UiNewRoomSingle", self.Data.StageId)
        end)
    else
        XLuaUiManager.Open("UiNewRoomSingle", self.Data.StageId)
