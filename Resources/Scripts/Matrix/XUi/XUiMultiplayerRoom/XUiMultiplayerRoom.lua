...
function XUiMultiplayerRoom:OnKickOut()
    ...
    if XUiManager.CheckTopUi(CsXUiType.Normal, "UiMultiplayerRoom") then
        self:Close()
    else
        self:Remove()
    end
...
function XUiMultiplayerRoom:InitCharItems()
    ...
    for i = 1, MAX_PLAYER_COUNT do
        ...
        ui.transform:Reset()
        local grid = XUiGridMulitiplayerRoomChar.New(ui, self, i, self.RoleModelList[i].RoleModel, self.RoleModelList[i].EffectObj)
...
function XUiMultiplayerRoom:RefreshButtonStatus()
    ...
    self.BtnMapSelect.gameObject:SetActiveEx(XDataCenter.FubenSpecialTrainManager.IsPhotoStage(roomData.StageId))
    self.PanelLimit.gameObject:SetActiveEx(not XDataCenter.FubenSpecialTrainManager.IsPhotoStage(roomData.StageId))
