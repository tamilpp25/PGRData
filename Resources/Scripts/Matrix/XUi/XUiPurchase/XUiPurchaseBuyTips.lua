...
-- 更新数据
function XUiPurchaseBuyTips:OnStart(data, checkBuyFun, updateCb, beforeBuyReqFun, uiTypeList,curType)
    if not data then
        return
    end
...
function XUiPurchaseBuyTips:CheckLBRewardIsHave()
    if self.Data.ConvertSwitch and self.Data.ConvertSwitch < self.Data.ConsumeCount and not self.Data.PayKeySuffix then -- 礼包存在已拥有物品折扣 --日服不判断礼包存在已拥有物品折扣
        ...
        if remainPrice == 0 then -- 全部都拥有
            ...
            local path = CS.XGame.ClientConfig:GetString("PurchaseBuyRiYuanIconPath1")
            self.RawImageConsume:SetRawImage(path)
        else -- 未拥有和拥有同时存在
            self.TxtHave.gameObject:SetActiveEx(true)
            self.TxtHave.text = TextManager.GetText("PurchaseLBHaveFashion")
...
function XUiPurchaseBuyTips:BuyPurchaseRequest()
    if self.Data.PayKeySuffix then
        ...
        if self.CurUiType ~= nil then
            XDataCenter.PayManager.Pay(key, 1, { self.Data.Id,self.CurUiType}, self.Data.Id)
        else
