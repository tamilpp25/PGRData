...
function XUiPurchaseCoatingLB:OpenUiView(data)
	...
	if templateId then
        ...
        if data.PayKeySuffix then
	        buyData.ItemIcon = CS.XGame.ClientConfig:GetString("PurchaseBuyRiYuanIconPath")
        else
            buyData.ItemIcon = XDataCenter.ItemManager.GetItemIcon(data.ConsumeId)
        end
        buyData.PayKeySuffix = data.PayKeySuffix
...
function XUiPurchaseCoatingLB:FashionDetailBuyCB()
    if self:CheckBuy() then
        if self.CurData.PayKeySuffix then --判断是不是直购礼包
            local key
