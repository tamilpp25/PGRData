...
--================
--获取羁绊现层数展示字符串(格式例子:"1/4")
--================
function XExpeditionCombo:GetCurrentPhaseStr()
    return string.format("%d/%d", self:GetComboActive() and self:GetPhase() or 0, self:GetPhaseNum())
end
--================
--获取羁绊图标路径
--================
...
--检查组合有效状态，返回状态结果
--@return phase:有效层数
--@return activeReferenceNum:达到下一层有效的关联角色数
--================
...
function XExpeditionCombo:ConditionMemberNum(index, tempList)
    ...
    local targetNum = 0
    return self.CheckListLength >= self:GetConditionNum(), self.CheckListLength
end
--================
--检查列表角色的总等级
--@param index:层数
