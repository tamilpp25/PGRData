...
PlayerCondition = {
    ...
    [10112] = function(condition) -- 单挑Boss奖励是否领取，活动是否开启
        ...
        if XFunctionManager.JudgeCanOpen(XFunctionManager.FunctionName.FubenActivitySingleBoss) and isFinish and isCurActivity then
            return true, condition.Desc
        end
        return false, condition.Desc
    end,
    ...
    [10130] = function(condition)--超级据点完成据点数量
        local requireCount = condition.Params[1]
        local haveCount = XDataCenter.StrongholdManager.GetFinishGroupCount()
        return haveCount >= requireCount, condition.Desc, haveCount, requireCount
    end,
    ...
    [10145] = function(condition) -- 判断超级爬塔通关目标关卡进度
        ...
        if not stageManager then return false end
        local stStage = stageManager:GetTargetStageByStStageId(targetStageId)
        if not stStage then return false end
    ...
    [11112] = function(condition)   --查询玩家是否拥有某个永久武器时装
        ...
        local ret = XDataCenter.WeaponFashionManager.CheckHasFashion(id)
        and not XDataCenter.WeaponFashionManager.IsFashionTimeLimit(id)
        return ret, condition.Desc
    end,
    [11113] = function(condition)   --查询玩家是否不拥有某个永久武器时装
        local id = condition.Params[1]
        local ret = not XDataCenter.WeaponFashionManager.CheckHasFashion(id)
        or XDataCenter.WeaponFashionManager.IsFashionTimeLimit(id)
        return ret, condition.Desc
    end,
    [11114] = function(condition)--查询玩家是否拥有一堆时装中的某一个
        local ret = false
        for _, fashionId in pairs(condition.Params) do
            ret = XDataCenter.WeaponFashionManager.CheckHasFashion(fashionId)
    ...
    [16100] = function(condition)   --查询公会等级是否达到（服务端未实现）
        if not XDataCenter.GuildManager.IsJoinGuild() then return false, condition.Desc end
    ...
    [10124] = function(condition)
        local wave = XDataCenter.ExpeditionManager.GetWave()
        return wave and wave >= condition.Params[1]
    end,
...
function XConditionManager.CheckCharacterCondition(id, characterId)
    ...
    if not func then
        XLog.Error("XConditionManager.CheckCharacterCondition error: can not found condition, id is " .. id .. " type is " .. template.Type)
        return DefaultRet
    end
...
function XConditionManager.CheckPlayerCondition(id, ...)
    ...
    if not func then
        XLog.Error("XConditionManager.CheckPlayerCondition error: can not found condition, id is " .. id .. " type is " .. template.Type)
        return DefaultRet
    end
...
function XConditionManager.CheckKillCondition(id, NpcId)
    ...
    if not func then
        XLog.Error("XConditionManager.CheckKillCondition error: can not found condition, id is " .. id .. " type is " .. template.Type)
        return DefaultRet
    end
...
function XConditionManager.CheckEquipCondition(conditionId, checkedId)
    ...
    if not func then
        XLog.Error("XConditionManager.CheckEquipCondition error: can not found condition, id is " .. conditionId .. " type is " .. template.Type)
        return DefaultRet
    end
