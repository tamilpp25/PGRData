XFightWordsManagerCreator = function()
    ...
    function XFightWordsManager.UnSchedule()
        if ScheduleId then
            CSXScheduleManagerUnSchedule(ScheduleId)
            ScheduleId = nil
        end
    end
    ...
    function XFightWordsManager.Resume()
        if TemplateId and PauseTime and NextTime then
            ...
            if (duration > 0) then
                ...
                ScheduleId = CSXScheduleManagerScheduleOnce(PlayNextWordFunc, duration)
            end
            PauseTime = nil
            NextTime = nil
    ...
    function XFightWordsManager.PlayNext()
        ...
        local duration = Template.Duration * 1000
        ...
        if cueId and cueId ~= 0 then
            if CurAudioInfo then
                CurAudioInfo:Stop()
            end
            CurAudioInfo = CS.XAudioManager.PlaySound(cueId)
        else
            CurAudioInfo = nil
        end
