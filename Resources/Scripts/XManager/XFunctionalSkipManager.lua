XFunctionalSkipManagerCreator = function()
    ...
    function XFunctionalSkipManager.OnOpenMainLineStage(list)
        ...
        if openStageDetail then
            ...
            CsXUiManager.Instance:Open("UiFubenMainLineChapter", chapter)
        end
    end
    ...
    function XFunctionalSkipManager.SkipToFubenCoupleCombat(list)
        ...
        local mode = list.CustomParams[1]
        local isOpen, desc = XDataCenter.FubenCoupleCombatManager.CheckModeOpen(mode)
        if not isOpen then
            XUiManager.TipMsg(desc)
            return
        end
        XLuaUiManager.Open("UiCoupleCombatMain", mode)
    end
