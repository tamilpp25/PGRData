...
function XUiBfrtRoomCharacter:OnBtnJoinTeamClick()
    local selectId = self.CurCharacterId

    local joinFunc = function(isReset)
        local echelonIndex, echelonType = self.CheckIsInTeamListCb(selectId)
        if echelonIndex and echelonType then
            local title = CS.XTextManager.GetText("BfrtDeployTipTitle")
            local characterName = XCharacterConfigs.GetCharacterName(selectId)
            ...
            XUiManager.DialogTip(title, content, XUiManager.DialogType.Normal, nil, function()
                ...
                self.CharacterSwapEchelonCb(selectId, self.TeamCharacterIdList[self.TeamSelectPos])
                self.TeamCharacterIdList[self.TeamSelectPos] = selectId
                self:Close()
            end)
        else
            if isReset then
