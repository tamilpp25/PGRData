...
local XUiExpeditionInfinityCombo = require("XUi/XUiExpedition/Battle/XUiExpeditionInfinityCombo")
...
function XUiExpeditionInfinityComboList:InitDynamicTable()
    ...
    self.DynamicTable:SetProxy(XUiExpeditionInfinityCombo)
...
function XUiExpeditionInfinityComboList:OnDynamicTableEvent(event, index, grid)
    ...
    elseif event == DYNAMIC_DELEGATE_EVENT.DYNAMIC_GRID_ATINDEX then
        if self.ComboList and self.ComboList[index] then
            grid:RefreshData(self.ComboList[index])
        end
    end
end
