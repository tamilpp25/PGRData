...
function XUiExpeditionInfinityWin:OnStart(data)
    ...
    self.HistoryWave = XDataCenter.ExpeditionManager.GetWave()
    local newWave = self.WinData.NpcGroup > 0 and self.WinData.NpcGroup or 0
    if self.HistoryWave < newWave then XDataCenter.ExpeditionManager.SetWave(self.WinData.NpcGroup) end
...
function XUiExpeditionInfinityWin:ShowWave()
    ...
    local newWave = self.WinData.NpcGroup > 0 and (self.WinData.NpcGroup - 1) or 0
