    ...
    for _, baseComboType in pairs(baseComboTypeCfgs) do
        ...
        for _, eCombo in pairs(self.ComboList) do
            if eCombo:GetComboTypeId() == baseId then
                if not comboList[baseId] then comboList[baseId] = {} end
                table.insert(comboList[baseId], eCombo)
            end
        end
    end
    local dataList = {}
    local tabCount = 1
    for baseId, childComboList in pairs(comboList) do
        local tabData = {
            TabType = TabType.First,
            Name = XExpeditionConfig.GetBaseComboTypeNameById(baseId),
            TabId = tabCount,
            BaseComboId = baseId
        }
