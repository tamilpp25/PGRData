...
function XUiGridBabelSelectTeam:Init(enterFightCb)
...
function XUiGridBabelSelectTeam:Refresh(stageId, teamId)
    ...
    local unlockTeamNum = XDataCenter.FubenBabelTowerManager.GetStageUnlockTeamNum()
    local isLock = teamId > unlockTeamNum + 1
    if not isLock then
...
function XUiGridBabelSelectTeam:OnClickBtnSelect()
    ...
    XLuaUiManager.Open("UiBabelTowerBase", self.StageId, self.TeamId, self.EnterFightCb)
end
