...
function XUiNewRoomSingle:OnStart(stageId, data, ...)
    ...
    self.Proxy = XUiNewRoomSingleProxy.ProxyDic[self.StageInfos.Type] or
    XUiNewRoomSingleProxy.ProxyDic[self.StageCfg.StageType]
...
function XUiNewRoomSingle:OnBtnTeamPrefabClick()
    ...
    XLuaUiManager.Open("UiRoomTeamPrefab", self.CurTeam.CaptainPos, self.CurTeam.FirstFightPos, characterLimitType, limitBuffId, self.StageCfg.StageType, teamGridId)
end
...
function XUiNewRoomSingle:OnBtnUnLockLongUp()
    ...
    if not self:CheckCharCanClickByProxy() then
        return
    end
    ...
    if self.HasRobot then
        ...
        elseif self:IsUnionKillType() then
            ...
            if XRobotManager.CheckIsRobotId(charId) then
                ...
                local robotCfg = XRobotManager.GetRobotTemplate(robotId)
                roleModelPanel:UpdateRobotModel(robotId, robotCfg.CharacterId, callback, robotCfg.FashionId, robotCfg.WeaponId)
            else
...
function XUiNewRoomSingle:RefreshCaptainSkill()
    ...
    if not XRobotManager.CheckIsRobotId(captainId) then
        if self.HasRobot or self.RogueLikeIsRobot then
            ...
            head = XDataCenter.CharacterManager.GetDefaultCharSmallHeadIcon(captainId)
        else
            -- 玩家角色
...
function XUiNewRoomSingle:IsChessPursuit()
    return self.StageCfg.StageType == XDataCenter.FubenManager.StageType.ChessPursuit
end
...
function XUiNewRoomSingle:InitChessPursuit()
    ...
    self.BtnTeamPrefab.gameObject:SetActiveEx(self.ChessPursuitData.SceneUiType == XChessPursuitCtrl.SCENE_UI_TYPE.BUZHEN)
...
function XUiNewRoomSingle:CheckCharCanClickByProxy()
    if self.Proxy and self.Proxy.CheckCanCharClick then
        return self.Proxy.CheckCanCharClick()
    end
