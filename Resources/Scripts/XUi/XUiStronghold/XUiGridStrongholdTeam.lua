...
function XUiGridStrongholdTeam:Ctor(ui, fightCb)
    ...
    self.BtnRune.CallBack = function() self:OnBtnRuneClick() end
...
function XUiGridStrongholdTeam:Refresh(teamList, teamId, groupId, isPrefab)
    self.TeamList = teamList--队伍数据更改赋值
    self.TeamListClip = XDataCenter.StrongholdManager.GetTeamListClipTemp(groupId, teamList)--仅显示用
    self.TeamId = teamId
...
function XUiGridStrongholdTeam:UpdateTeam()
    ...
    for index = 1, requireMemberNum do
        ...
        if not grid then
            local go = index == 1 and self.GridDeployMember or CSUnityEngineObjectInstantiate(self.GridDeployMember, self.PanelDeployMembers)
...
function XUiGridStrongholdTeam:UpdatePlugins(doNotShowEffect)
    ...
    for index = 1, #plugins do
        ...
        if not grid then
            local go = index == 1 and self.GridPlugin or CSUnityEngineObjectInstantiate(self.GridPlugin, self.PanelCoreContent)
...
function XUiGridStrongholdTeam:OnBtnLeaderClick()
    ...
    XLuaUiManager.Open("UiNewRoomSingleTip", self, characterIdList, captainPos, function(index)
        ...
        self:UpdateTeam()
    end, characterIdToIsIsAssitantDic)
end
...
function XUiGridStrongholdTeam:OnBtnRuneClick()
    ...
    XLuaUiManager.Open("UiStrongholdRune", self.TeamList, self.TeamId, self.GroupId)
end
