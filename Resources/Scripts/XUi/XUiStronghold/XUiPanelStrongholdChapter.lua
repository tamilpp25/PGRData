...
local ANIM_DURATION = 0.2--关卡滑动动画持续时间
local CENTER_PERCENT = 0.3--关卡要滑动到的位置占屏幕宽度百分比
...
function XUiPanelStrongholdChapter:Refresh(chapterId)
    ...
    for index, groupId in ipairs(groupIds) do
        ...
        if not grid then
            ...
            if not parent then
                XLog.Error("XUiPanelStrongholdChapter:Refresh error:stage num error: chapterId is: " .. chapterId .. ", prefabPath: " .. prefabPath .. ", groupIds:", groupIds)
                return
            end
...
function XUiPanelStrongholdChapter:OnClickStage(gridIndex)
    ...
    if self.ClickStageCb then self.ClickStageCb(groupId) end
