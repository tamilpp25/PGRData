...
function XLevelGuide1:FinishedGuideStep(step)
    ...
    elseif step == 5 then
        self._proxy:SetUiWidgetActive(EUiIndex.Fight, EUiFightWidgetKey.BtnFocus, false)
        --召唤boss
        self._bossId = self._proxy:GenerateNpc(8004, 2, self._resetBossPosition1, self._resetBossRotation1)
        self:ShowTip(12041)
    elseif step == 6 then
    elseif step == 7 then
...
function XLevelGuide1:ReGenerateNpc(case)
    self._bossId = self._proxy:GenerateNpc(8004, 2, self._bossDiePosition, self._resetBossRotation1) --在玩家看不见的地方召唤新的boss
