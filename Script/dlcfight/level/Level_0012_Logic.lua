...
local XLevelGuide1 = XDlcScriptManager.RegLevelLogicScript(0012, "XLevelGuide1")
local Config = require("Level/LevelConfig/Hunt02GuideConfig") -- 读取场景物体的配置数据,作为实例存在本地
local Tool = require("Level/Common/XLevelTools")
local Timer = require("Level/Common/XTaskScheduler")

function XLevelGuide1.Reload()
    Config = require("XDLCFight/Level/LevelConfig/Hunt02GuideConfig")
end
...
function XLevelGuide1:Init()
    ...
    --开关关掉多余UI
    --self._proxy:SetUiWidgetActive(EUiIndex.Fight, EUiFightWidgetKey.BtnJump, false)
    ...
    self._proxy:SetUiActive(EUiIndex.OnlineMsg, false)

    --关闭第五步的物体
    self._proxy:SetSceneObjectActive(5, false)
    self._proxy:SetSceneObjectActive(6, false)
    XLog.Debug("<color=#F0D800>[SceneHunt02]</color>关卡流程设置完成")

    if self._initProgress >= 1 then --跳阶段
        self._curProgress = self._initProgress - 1
        self:FinishedGuideStep(self._curProgress) --从_curProgress的末尾（完成状态）开始，去触发要测试的下一阶段
        local pos = { x = 16.6, y = 4, z = -118.6 }
        self._proxy:SetNpcPosition(self._localPlayerNpcId, pos)
    end
...
function XLevelGuide1:Update(dt)
    ...
    if self._debug then
        ...
        elseif self._proxy:IsKeyDown(ENpcOperationKey.Ball9) then
            self._proxy:AddSkillBall(self._localPlayerNpcId, 0, 3)
        end
    end
end
...
function XLevelGuide1:HandleEvent(eventType, eventArgs)
    ...
    if eventType == EWorldEvent.SceneObjectTrigger then
        ...
        if eventArgs.SourceActorId == self._localPlayerNpcId then
            if self._curProgress == 1 then
                if eventArgs.SceneObjectId == self._jumpGuideTrigger
                    and eventArgs.TriggerId == 1
                    and eventArgs.TriggerState == ESceneObjectTriggerState.Enter then
                    --and not self._levelProgress[2].start
                    --开启跳跃按钮
                    self:StartGuideStep(2)
                end
            elseif self._curProgress == 2 then
                if eventArgs.SceneObjectId == self._jumpSucTrigger
                    and eventArgs.TriggerId == 1
                    and eventArgs.TriggerState == ESceneObjectTriggerState.Enter then
            ...
            elseif self._curProgress == 4 then
                if eventArgs.SceneObjectId == 4
                    and eventArgs.TriggerId == 2
                    and eventArgs.TriggerState == ESceneObjectTriggerState.Enter and not self._levelProgress[5].start then
            ...
            elseif self._curProgress == 5 then
                if eventArgs.SceneObjectId == self._step5AnchorTrigger
                    and eventArgs.TriggerId == 1
                    and eventArgs.TriggerState == ESceneObjectTriggerState.Enter and not self._levelProgress[5].highGround then
                    --step5勾上高地
                    self._proxy:CreateLevelEffect(52, "FxTDSummonPoint02", 21.26, 9.05, -88, 0, 0, 0, 0, 0, 0)
                    --Tool.ShowGuide(self._guider, 43)
                    Tool.NextGuide(self._guider)
                    self._levelProgress[5].highGround = true
                    --润色：延时任务提示前往目标
                elseif eventArgs.SceneObjectId == self._step5EndPosTrigger
                    and eventArgs.TriggerId == 1
                    and eventArgs.TriggerState == ESceneObjectTriggerState.Enter and not self._levelProgress[5].endPointListen then
                    --step5到达终点
                    if self._proxy:CheckLevelEffectExist(52) then
                        self._proxy:RemoveLevelEffect(52)
                    end
                    self._levelProgress[5].endPointListen = true
                    Tool.ShowGuide(self._guider, 45)
                    self._proxy:CreateLevelEffect(54, "FxTDSummonPoint02", 5.9, 3.98, -119.43, 0, 0, 0, 0, 0, 0)
                end
                if eventArgs.SceneObjectId == self._step6StartTrigger
                    and eventArgs.TriggerId == 1
                    and eventArgs.TriggerState == ESceneObjectTriggerState.Enter then
            ...
            elseif self._curProgress == 6 then
                --[[                if eventArgs.SceneObjectId == self._step7NearBossTrigger
                                        and eventArgs.TriggerId == 1
                                        and eventArgs.TriggerState == ESceneObjectTriggerState.Enter and not self._levelProgress[7].start then
            ...
            elseif self._curProgress == 8 then
                if eventArgs.SceneObjectId == self._step8GroundTrigger
                    and eventArgs.TriggerId == 1
                    and eventArgs.TriggerState == ESceneObjectTriggerState.Enter
            ...
            --关卡结束判断
            if self._exitActive and eventArgs.SceneObjectId == 9999 and eventArgs.TriggerId == 1 then
                self._proxy:FinishFight(true)
            end
        end

        if eventArgs.SceneObjectId == self._deathZonePlaceId
            and eventArgs.TriggerId == 1
            and self._proxy:IsPlayerNpc(eventArgs.SourceActorId) then
            self._proxy:ResetNpcToSafePoint(eventArgs.SourceActorId)
        end
    end
    if eventType == EWorldEvent.NpcCastSkill
        and eventArgs.LauncherId == self._localPlayerNpcId then
    ...
    elseif eventType == EWorldEvent.NpcExitSkill
        and eventArgs.LauncherId == self._localPlayerNpcId then
...
function XLevelGuide1:StartGuideStep(step)
    ...
    if step == 1 then
    elseif step == 2 then
        self._proxy:SetUiWidgetActive(EUiIndex.Fight, EUiFightWidgetKey.BtnJump, true)
        if self._proxy:CheckLevelEffectExist(11) then
            self._proxy:RemoveLevelEffect(11)
        end
        --润色：延时任务再次提示使用跳跃按键
    elseif step == 3 then
        --润色：延时任务再次提示攻击开关
    ...
    elseif step == 6 then
        --step6开始那个门
        if self._proxy:CheckLevelEffectExist(54) then
            self._proxy:RemoveLevelEffect(54)
        end
        self:CloseTip()
    elseif step == 7 then
...
function XLevelGuide1:FinishedGuideStep(step)
    ...
    if step == 1 then
        ...
        self._proxy:SetUiWidgetActive(EUiIndex.Fight, EUiFightWidgetKey.BtnFocus, false)
        --创建目的地特效
        self._proxy:CreateLevelEffect(11, "FxTDSummonPoint02", 27, 1.987, -13, 0, 0, 0, 0, 0, 0)
    elseif step == 2 then
        self._proxy:SetUiWidgetActive(EUiIndex.Fight, EUiFightWidgetKey.BtnSpear, false)
        self._proxy:SetUiWidgetActive(EUiIndex.Fight, EUiFightWidgetKey.BtnFocus, false)
    elseif step == 3 then
        self._proxy:RemoveNpcFocusTarget(self._localPlayerNpcId)
        self._proxy:SetUiWidgetActive(EUiIndex.Fight, EUiFightWidgetKey.BtnSpear, false)
        self._proxy:SetUiWidgetActive(EUiIndex.Fight, EUiFightWidgetKey.BtnFocus, false)
        self:ShowTip(12020)
    elseif step == 4 then
    elseif step == 5 then
        self._proxy:SetUiWidgetActive(EUiIndex.Fight, EUiFightWidgetKey.BtnFocus, false)
        --召唤boss
        self._bossId = self._proxy:GenerateNpc(1, 8004, 2, self._resetBossPosition1, self._resetBossRotation1)
        self:ShowTip(12041)
    elseif step == 6 then
    elseif step == 7 then
    ...
    elseif step == 8 then
        ...
        self._proxy:RemoveBuff(self._localPlayerNpcId, 8004018)
    elseif step == 9 then
    elseif step == 10 then
    end
...
function XLevelGuide1:GateControl()
    if self._gateOpen then
        self._proxy:PlaySceneAnimation(0)
    else
        self._proxy:PlaySceneAnimation(1)
    end
...
function XLevelGuide1:SetExitActive(active)
    if active and not self._exitActive then
        ...
        if not self._proxy:CheckLevelEffectExist(9999) then
            self._proxy:CreateLevelEffect(9999, "FxExitGate", -53.994, 5.822186, -125.7642, 0, 90, 0, 0, 0, 0)
        end
    elseif not active and self._exitActive then
        XLog.Debug("<color=#F0D800>[SceneHunt02]</color>关卡退出出口关闭")
        if self._proxy:CheckLevelEffectExist(9999) then
            self._proxy:RemoveLevelEffect(9999)
        end
    end
...
function XLevelGuide1:FocusOnSceneObj(placeId)
    ...
    local uuid = self._proxy:GetSceneObjectUUID(placeId)
    self._proxy:SetNpcFocusTarget(self._localPlayerNpcId, uuid)
end
---注视npc
function XLevelGuide1:FocusOnNpc()
    self._proxy:SetNpcFocusTarget(self._localPlayerNpcId, self._bossId)
end
---取消注视
...
function XLevelGuide1:CloseTip()
    if self._tipRef ~= nil then
        self._proxy:CloseTip(self._tipRef)
        self._tipRef = nil
    end
end
---第四步启用锚点
...
function XLevelGuide1:Step4ListenFocus(dt)
    local camLockAnchor = self._proxy:GetNpcNoteInt(self._localPlayerNpcId, self._camLockAnchorIndex)
...
function XLevelGuide1:Step4EnableAnchorListen()
    self._proxy:SetUiWidgetActive(EUiIndex.Fight, EUiFightWidgetKey.BtnSpear, true)
    local camLockAnchor = self._proxy:GetNpcNoteInt(self._localPlayerNpcId, self._camLockAnchorIndex)
...
function XLevelGuide1:Step5ObjectReveal()
    self._proxy:SetSceneObjectActive(5, true)
    self._proxy:SetSceneObjectActive(6, true)
    self._proxy:CreateLevelEffect(50, "FxQte", 39.86927, 5.046208, -64.46697, 0, 0, 0, 0, 0, 0)
    self._proxy:CreateLevelEffect(51, "FxQte", 41.96107, 8.160282, -69.77538, 90, 0, 0, 0, 0, 0)
end
...
function XLevelGuide1:Step6ShowFocusBtn()
    self._proxy:SetNpcNoteInt(self._localPlayerNpcId, 200, 1)
    self._proxy:SetUiWidgetActive(EUiIndex.Fight, EUiFightWidgetKey.BtnFocus, true)
end
---第七步教学 攻击练习
...
function XLevelGuide1:Step7Update(dt)
    if self._proxy:CheckBuffByKind(self._bossId, 8004021) and not self._levelProgress[7].stunGuided then
        XLog.Debug("<color=#F0D800>[教学关]</color>检测到怪物硬直")
        self._proxy:RemoveBuff(self._bossId, 8004021)
        Tool.ShowGuide(self._guider, 63)
        self._levelProgress[7].stunGuided = true
    end
    if self._proxy:CheckBuffByKind(self._bossId, 8004029) then
        if self:CheckBlueLock() then
            self._proxy:RemoveBuff(self._bossId, 8004029)
            self._timer:Schedule(0.7, self._guider, Tool.ShowGuide, 64)
            self._levelProgress[7].attackPractice = false --关闭攻击练习，部位破坏的监控
            self._proxy:ClearAllSkillBalls(self._localPlayerNpcId)
            XLog.Debug("<color=#F0D800>[教学关]</color>检测到怪物部位被破坏了")
        else
            -- 要求蓝锁
        end
    end
    if self._proxy:GetSkillBallCount(self._localPlayerNpcId) <= 3 then
    ...
    if self._levelProgress[7].stageTime >= self._levelProgress[7].giveBallCD then
        ...
        if math.random(100) > 50 then
            self._proxy:AddSkillBall(self._localPlayerNpcId, 2, 3)
            self._proxy:AddSkillBall(self._localPlayerNpcId, 3, 1)
        else
            self._proxy:AddSkillBall(self._localPlayerNpcId, 3, 3)
            self._proxy:AddSkillBall(self._localPlayerNpcId, 2, 1)
        end
        self._proxy:AddSkillBall(self._localPlayerNpcId, 1, 3)
    end
end
---第八步教学 期间每帧监控
function XLevelGuide1:Step8Listen(dt)
    if self._levelProgress[8].Fly then
        if self._proxy:CheckBuffByKind(self._localPlayerNpcId, 8004022) then
            self._proxy:RemoveBuff(self._localPlayerNpcId, 8004022)
            ...
            self._proxy:ClearAllSkillBalls(self._localPlayerNpcId)
            --玩家攻击力扣到0
            self._proxy:AddBuff(self._localPlayerNpcId, 8004018)
        end
    elseif self._levelProgress[8].NormAtk then
        if self._proxy:CheckBuffByKind(self._localPlayerNpcId, 8004025) then
            self._proxy:RemoveBuff(self._localPlayerNpcId, 8004025)
            ...
            --普攻发球
            self._proxy:AddSkillBall(self._localPlayerNpcId, 1, 1)
            if self._levelProgress[8].AtkCount >= 6 then
                ...
                self._timer:Schedule(1, self._localPlayerNpcId, function(npc, id) self._proxy:RemoveBuff(npc, id) end, 8004026)
            end
            self:ShowTip(self._tipRef, self._levelProgress[8].AtkCount)
        end
    elseif self._levelProgress[8].Eliminate then
        if self._proxy:CheckBuffByKind(self._localPlayerNpcId, 8004026) then
            self._proxy:RemoveBuff(self._localPlayerNpcId, 8004026)
            ...
            if self._levelProgress[8].EliminateCount >= 3 then
                ...
                self._timer:Schedule(1, self, self.CloseTip)
                self._timer:Schedule(1, self._localPlayerNpcId, function(npc, id)self._proxy:RemoveBuff(npc, id)  end, 8004023)
            elseif self._proxy:GetSkillBallCount(self._localPlayerNpcId) <= 1 then
    ...
    elseif self._levelProgress[8].Zuma then
        if self._proxy:CheckBuffByKind(self._localPlayerNpcId, 8004023) then
            self._proxy:RemoveBuff(self._localPlayerNpcId, 8004023)
            ...
            if self._levelProgress[8].ZumaCount >= 1 then
                ...
                self._timer:Schedule(0.5, self._localPlayerNpcId, function(npc, id)self._proxy:RemoveBuff(npc, id) end, 8004024)
                self._proxy:AddSkillBall(self._localPlayerNpcId, 1, 2)
                self._proxy:AddSkillBall(self._localPlayerNpcId, 3, 3)
                self._proxy:AddSkillBall(self._localPlayerNpcId, 2, 1)
                self._proxy:AddSkillBall(self._localPlayerNpcId, 1, 1)
                self._proxy:AddSkillBall(self._localPlayerNpcId, 3, 2)
                self._proxy:AddSkillBall(self._localPlayerNpcId, 1, 2)
            end
            self:ShowTip(self._tipRef, self._levelProgress[8].ZumaCount)
        elseif self._proxy:GetSkillBallCount(self._localPlayerNpcId) <= 3 and self._proxy:CheckBuffByKind(self._localPlayerNpcId, 8004026) then
            self._proxy:RemoveBuff(self._localPlayerNpcId, 8004026)
            --祖玛失败的处理 再发一次球
            self._proxy:ClearAllSkillBalls(self._localPlayerNpcId)
            self._proxy:AddSkillBall(self._localPlayerNpcId, 1, 2)
    ...
    elseif self._levelProgress[8].Color then
        if self._proxy:CheckBuffByKind(self._localPlayerNpcId, 8004024) then
            self._proxy:RemoveBuff(self._localPlayerNpcId, 8004024)
            ...
            if self._levelProgress[8].ColorCount >= 1 then
                ...
                self._proxy:RemoveBuff(self._localPlayerNpcId, 1000069)
            end
        elseif self._proxy:GetSkillBallCount(self._localPlayerNpcId) <= 1 then
            self._proxy:AddSkillBall(self._localPlayerNpcId, 1, 2)
            self._proxy:AddSkillBall(self._localPlayerNpcId, 3, 3)
...
function XLevelGuide1:Step8WaitingEliminate()
    ...
    self._proxy:AddSkillBall(self._localPlayerNpcId, 1, 2)
    self._proxy:AddSkillBall(self._localPlayerNpcId, 2, 3)
    self._proxy:AddSkillBall(self._localPlayerNpcId, 3, 4)
end
---第八步教学 上天检测，祖玛引导
...
function XLevelGuide1:Step9Listen(dt)
    if not self._levelProgress[9].success then
        ...
        if self._levelProgress[9].blueLockListen then
            if --[[self._proxy:CheckBuffByKind(self._bossId, 1000000) and]] self._proxy:CheckBuffByKind(self._bossId, 8004029) then
                self._proxy:RemoveBuff(self._bossId, 8004029)
        ...
        --成功失败检测
        if self._proxy:CheckBuffByKind(self._bossId, 8004027) then
            XLog.Debug("<color=#F0D800>[教学关]</color>检测到钉入成功")
            self._proxy:RemoveBuff(self._bossId, 8004027)
            self._levelProgress[9].success = true
            self:FinishGuide()
            self:CloseTip()
            return
        elseif self._proxy:CheckBuffByKind(self._bossId, 8004028) then
            XLog.Debug("<color=#F0D800>[教学关]</color>检测到钉入失败")
            self._proxy:RemoveBuff(self._bossId, 8004028)
...
function XLevelGuide1:ResetFight(case)
    ...
    self._timer:Schedule(2, self._localPlayerNpcId, function(npc, id)self._proxy:RemoveBuff(npc, id)  end, 5000003) --黑屏2s

    if self._bossId ~= nil and self._proxy:CheckNpc(self._bossId) then
        --die会持续一段时间，以后需要换成remove
        self._timer:Schedule(0.5, self._bossId, function(npc, p)self._proxy:SetNpcPosition(npc, p) end, self._bossDiePosition) -- 把boss传送到看不见的地方
        self._timer:Schedule(0.5, self._bossId, function(npc)self._proxy:NpcDie(npc) end)
        self:AddBuff(self._localPlayerNpcId, 8004015)--隐藏猎锚及其特效
        self._proxy:RemoveNpcFocusTarget(self._localPlayerNpcId) --解除锁定
    end
...
function XLevelGuide1:ReGenerateNpc(case)
    self._bossId = self._proxy:GenerateNpc(1, 8004, 2, self._bossDiePosition, self._resetBossRotation1) --在玩家看不见的地方召唤新的boss
...
function XLevelGuide1:ResetPosition(case)
    self._proxy:SetNpcPosAndRot(self._localPlayerNpcId, self._resetNpcPosition, self._resetNpcRotation)
    if case == 1 then
        self._proxy:SetNpcPosAndRot(self._bossId, self._resetBossPosition1, self._resetBossRotation1)
    elseif case == 2 then
        self._proxy:SetNpcPosAndRot(self._bossId, self._resetBossPosition2, self._resetBossRotation2)
    end
end
---log并加buff，替换直接加buff
function XLevelGuide1:AddBuff(npc, buffId)
    ...
    self._proxy:AddBuff(npc, buffId)
end
---自动黄锁
function XLevelGuide1:AutoYellowLock()
    --如果玩家没有lock怪物，那就帮他lock
    local npcCamLock0 = self._proxy:GetNpcNoteInt(self._localPlayerNpcId, 2402) ~= 0
    local npcCamLock1 = self._proxy:GetNpcNoteInt(self._localPlayerNpcId, 2102) ~= 0
    if not (npcCamLock0 or npcCamLock1) then
        XLog.Debug("<color=#F0D800>[教学关]</color>帮玩家锁定怪物")
        self._proxy:SetNpcNoteInt(self._localPlayerNpcId, 30011, self._bossId) --设置目标
        self._proxy:SetNpcNoteInt(self._localPlayerNpcId, 2409, 3) --模拟手动点击
    end
end
---蓝锁检测
function XLevelGuide1:CheckBlueLock()
    return self._proxy:CheckBuffByKind(self._bossId, 1000000) or self._proxy:CheckBuffByKind(self._bossId, 1000001) or self._proxy:CheckBuffByKind(self._bossId, 1000002) or self._proxy:CheckBuffByKind(self._bossId, 1000003)
end
