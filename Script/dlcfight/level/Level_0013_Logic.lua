...
function XLevelBossFightHunt01:HandleEvent(eventType, eventArgs)
    ...
    if eventType == EWorldEvent.SceneObjectTrigger then
        ...
        --        .. " Log自关卡"
        --)
        if eventArgs.SceneObjectId == self._deathZonePlaceId and self._proxy:IsPlayerNpc(eventArgs.SourceActorId) then --死亡区域
            self._proxy:ResetNpcToSafePoint(eventArgs.SourceActorId)
        end
        if eventArgs.SceneObjectId == self._outwardsDeathZonePlaceId
            and self._proxy:IsPlayerNpc(eventArgs.SourceActorId)
            and eventArgs.TriggerState == ESceneObjectTriggerState.Exit then --外部死亡区域，一个临时解决方案

            self._proxy:ResetNpcToCheckPoint(eventArgs.SourceActorId)
...
function XLevelBossFightHunt01:InitBlackDragon()
    local id = self._proxy:GetGeneratedNpc(self._blackDragonBossRefId)
    --生成怪物
    if id <= 0 then
        --防止断线重连时重复创建怪物
        self._blackDragonNpcId = self._proxy:GenerateNpc(self._blackDragonBossRefId, self._blackDragonRespawnId, ENpcCampType.Camp2, self._bossBornPos, self._bossBornRot)
    else
...
function XLevelBossFightHunt01:OnNpcInteractComplete(eventArgs)
    ...
    if target then
        self._proxy:RebornNpc(npc, target) --复活救援对象
        self._playerRescueDict[npc] = nil --清除救援对象记录
    else
        XLog.Debug(string.format("Level interaction complete (Npc:%d), but no target to reborn.", npc))
    end
    self._proxy:CloseInteraction(npc, false) --关闭救援者的交互按钮
end
...
function XLevelBossFightHunt01:OnUpdatePhase(dt)
    ...
    elseif self._currentPhase == 1 then
        ...
        if self._bossIsReady and self._bossHasCastSkill and self._proxy:CheckCanCastSkill(self._blackDragonNpcId) then
            self:SetPhase(2)
        end
    ...
    elseif self._currentPhase == 2 then
        if eventType == EWorldEvent.NpcCastSkill then
            if eventArgs.SkillId == 100241 or eventArgs.SkillId == 101439 or eventArgs.SkillId == 100736 then
                ...
                self._playerFocusAnchorDic[eventArgs.LauncherId] = self._proxy:GetNpcNoteInt(eventArgs.LauncherId, 25003)
...
function XLevelBossFightHunt01:TestSwitchStartJumpFun()
    if self._proxy:CheckNpc(self._blackDragonNpcId) then
        local bossState = self._proxy:GetNpcNoteInt(self._blackDragonNpcId, 5001) --boss状态层
        local bossAngry = self._proxy:GetNpcNoteInt(self._blackDragonNpcId, 50) --boss愤怒状态
...
function XLevelBossFightHunt01:JumpFunStartCheck()
    ...
    ---boss没有狂暴
    if self._proxy:CheckNpc(self._blackDragonNpcId) then
        local bossHpRate = self._proxy:GetNpcAttribRate(self._blackDragonNpcId, 0) --boss血量百分比
        local bossState = self._proxy:GetNpcNoteInt(self._blackDragonNpcId, 5001) --boss状态层
        local bossAngry = self._proxy:GetNpcNoteInt(self._blackDragonNpcId, 50) --boss愤怒状态
...
function XLevelBossFightHunt01:LetBossReady()
    if self._proxy:CheckNpc(self._blackDragonNpcId) then
        self._proxy:SetNpcNoteFloat3(self._blackDragonNpcId, 9001, 60, 0, 60)
        self._proxy:ApplyMagic(self._blackDragonNpcId, self._blackDragonNpcId, self._bossEndureMagicId, 1)
    end
    -- 通知玩家跳跳乐开始
    for _, npc in pairs(self._playerNpcList) do
        self._proxy:ApplyMagic(npc, npc, self._jumpFunStartSignMagicId, 1)
    end

    --boss无视跳跳乐用的弹出collider
    self._proxy:SetSceneColliderIgnoreCollision(self._blackDragonNpcId, "DynamicBlock", -1, true)

    --场地中央开始把玩家弹开
    self._proxy:PlaySceneAnimation(5)
end

---对坐标点放技能
function XLevelBossFightHunt01:BossCastReadySwoop()
    if self._proxy:CheckNpcAction(self._blackDragonNpcId, ENpcAction.Move) and self._proxy:CheckCanCastSkill(self._blackDragonNpcId) then
        self._proxy:CastSkillToPosition(self._blackDragonNpcId, 8001097, 60, 0, 60)
...
function XLevelBossFightHunt01:StartJumpFun()
    ...
    --怪物大吼
    self._proxy:CastSkill(self._blackDragonNpcId, 8001001)
    ...
    --关闭弹出玩家的碰撞体
    self._timer:Schedule(8, 6, function(id) self._proxy:PlaySceneAnimation(id) end)

    --开始QTE
    self._proxy:UpdateRoleCountQTETime(15, 10, 5)
    self._timer:Schedule(3, 30, function(time) self._proxy:AddQTETime(time) end)
...
function XLevelBossFightHunt01:BossRoarMissile()
    if self._proxy:CheckNpc(self._blackDragonNpcId) then
        for _, npc in pairs(self._playerNpcList) do
            if self._proxy:CheckNpc(npc) then
                self._proxy:LaunchMissile(self._blackDragonNpcId, npc, 800100101)
...
function XLevelBossFightHunt01:BigSkill(param)
    if self._proxy:CheckNpc(self._blackDragonNpcId) then
        ...
        self._proxy:SetNpcNoteInt(self._blackDragonNpcId, 90000, param)
        --XLog.Debug("<color=#F0D800>[SceneHunt01]</color>验证boss 90000 = " .. self._proxy:GetNpcNoteInt(self._blackDragonNpcId,90000))
    end
end
...
function XLevelBossFightHunt01:PlayerSetAnchorToBoss(playerId)
    ...
    --[[    if completeCount >= 3 then
            self._timer:Schedule(3,4,self._proxy:SetPhase)
        end]]
end
...
function XLevelBossFightHunt01:OnJumpFunEnd()
    ...
    self._proxy:UpdateRoleCountQTETime(-1, -1, -1)
end
...
function XLevelBossFightHunt01:OnBossFlyEnd()
    --玩家回复锁定能力
    for _, npc in pairs(self._playerNpcList) do
        self._proxy:ApplyMagic(npc, npc, self._jumpFunEndSignMagicId, 1) --to remove jump fun start sign
        XLog.Debug("<color=#F0D800>[SceneHunt01]</color>npc" .. npc .. "恢复锁定能力")
    end
    --释放boss
    if self._proxy:CheckNpc(self._blackDragonNpcId) then
        self._proxy:SetNpcNoteFloat3(self._blackDragonNpcId, 9001, 0, 0, 0)
        self._proxy:ApplyMagic(self._blackDragonNpcId, self._blackDragonNpcId, self._removeBossEndureMagicId, 1)
    end
...
function XLevelBossFightHunt01:CenterTowerControl(raise)
    ...
    if raise then
        ...
        self._timer:Schedule(2.3, self._centerTowerEffectPlayer,
            function(so, arg) self._proxy:DoSceneObjectAction(so, arg) end, 2001) --中央塔升起结束特效
    else
        self._proxy:PlaySceneAnimation(2)
        self._timer:Schedule(3, self, self.OnCenterTowerMoveEnd)
        self._timer:Schedule(2.7, self._centerTowerEffectPlayer,
            function(so, arg) self._proxy:DoSceneObjectAction(so, arg) end, 2001) --中央塔降下结束特效
    end

    self._proxy:DoSceneObjectAction(self._centerTowerEffectPlayer, 2002) --开始移动的特效
