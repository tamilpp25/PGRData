...
---@param eventArgs table @XNpcInteractCompleteEventArgs
function XLevelBossFightHunt01:OnNpcInteractComplete(eventArgs)
    local launcherId = eventArgs.LauncherId
    local targetId = self._playerRescueDict[launcherId]
    if targetId then
        self._proxy:RebornNpc(launcherId, targetId) --复活救援对象
        self._playerRescueDict[launcherId] = nil --清除救援对象记录
    else
        XLog.Debug(string.format("Level interaction complete (Npc:%d), but no target to reborn.", launcherId))
    end
    self._proxy:CloseInteraction(launcherId, false) --关闭救援者的交互按钮
end
