...
function XLevelScript100:Init()
    ...
    self._endTime = self._levelId == 1011 and 90 or 120 --关卡限时
    ...
    self._monNpcId = nil
    self._monNpcId = self._proxy:GenerateNpc(1, monTableId, 2, monPosition, monRotation)
...
function XLevelScript100:HandleEvent(eventType, eventArgs)
    ...
    if (eventType == EWorldEvent.SceneObjectTrigger) then
        ...
        if (eventArgs.SceneObjectId == self._triggerOfFalling and eventArgs.TriggerState == 1)  then
            ...
            if (eventArgs.SourceActorId == self._playerNpcList[1]) then
                ...
                self._stunScore[1] = self._stunScore[1] + self._falingValue  --坠落扣分
                self._playerScore[1] = self._playerScore[1] - self._falingValue  --个人分数处理
                self._proxy:ChangeRubikPlayerScore(self._playerNpcList[1], -self._falingValue)
            ...
            if (eventArgs.SourceActorId == self._playerNpcList[2]) then
                ...
                self._stunScore[2] = self._stunScore[2] + self._falingValue
                self._playerScore[2] = self._playerScore[2] - self._falingValue  --个人分数处理
                self._proxy:ChangeRubikPlayerScore(self._playerNpcList[2], -self._falingValue)
            ...
            if (eventArgs.SourceActorId == self._playerNpcList[3]) then
                ...
                self._stunScore[3] = self._stunScore[3] + self._falingValue
                self._playerScore[3] = self._playerScore[3] - self._falingValue  --个人分数处理
                self._proxy:ChangeRubikPlayerScore(self._playerNpcList[3], -self._falingValue)
            ...
            self._proxy:SetNpcPosAndRot(self._triggerNpc, self._respawnPositionFall[math.random(1, 4)], self._respawnRotationFall, true)
        end
    end
...
function XLevelScript100:Update(dt)
    ...
    if self._levelId ~= 1011 then  --非普通难度则生成道具
...
function XLevelScript100:PlayerSkillJudge()
    ...
    for index, table in ipairs(self._explosionGridsOfPlayers) do  --再检测boss是否中了每个玩家的招w(ﾟДﾟ)w
        for _, value in ipairs(table) do  
...
function XLevelScript100:GetExplosionGrids(gridIndex, skill)
...
function XLevelScript100:LightUpPlane(playerIndex, tileTable, effectName)
    ...
    for index, value in ipairs(tileTable) do 
        ...
        self._timer:Schedule(delayTime, self, function()
...
function XLevelScript100:FinishLevel()
    ...
    for  index,playNpc in ipairs(self._playerNpcList) do
        self._proxy:ApplyMagic(playNpc, playNpc, 200024, 1)    --finish特效    
        self._proxy:SetFightResultCustomData(playNpc, 1, self._playerScore[index])    --个人积分
        self._proxy:SetFightResultCustomData(playNpc, 2, self._teamScore)     --队伍积分
        self._proxy:SetFightResultCustomData(playNpc, 3, self._attackScore[index])    --命中得分
        self._proxy:SetFightResultCustomData(playNpc, 4, self._stunScore[index])    --受击扣分
        self._proxy:SetFightResultCustomData(playNpc, 5, self._extraScore[index])    --合作加分
        self._proxy:SetFightResultCustomData(playNpc, 6, self._attackTimes[index])    --攻击boss次数
        self._proxy:SetFightResultCustomData(playNpc, 7, self._pickupTimes[index])    --拾取道具次数
        self._proxy:SetFightResultCustomData(playNpc, 8, self._saveTimes[index])    --救人次数
        self._proxy:SetFightResultCustomData(playNpc, 9, self._stunTimes[index])    --眩晕次数
        self._proxy:SetFightResultCustomData(playNpc, 10, self._fallingTimes[index])    --坠落复活次数
    end
    self._proxy:DestroyAllMissileDependOnLauncher(self._monNpcId)  --移除boss的所有子弹
    self._timer:Schedule(2, self, self._proxy:FinishFight(true))  --"已经结束啦！"
end
