    ...
    elseif self._currentPhase == 29 then
        if self:OnInteractButton() then
          --"对局内，我们可以救助晕眩中的队友。"
            ...
            --判断离13和16号地板中心哪个远
            local distance1 = self:CalculateTheDistance(playerNpcPosition.x, playerNpcPosition.z, self._effectPositionX[13], self._effectPositionZ[13])
            local distance2 = self:CalculateTheDistance(playerNpcPosition.x, playerNpcPosition.z, self._effectPositionX[16], self._effectPositionZ[16])
...
function XLevelScript1031:FinishLevel()
    ...
    self._timer:Schedule(2, self._proxy, self._proxy.FinishFight, true)  --"已经结束啦！"
end
...
function XLevelScript1031:CalcExplosionGrids(grid, skillID)
