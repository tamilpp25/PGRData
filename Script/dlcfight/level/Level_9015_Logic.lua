...
function XLevel9015:InitNpc()
    --生成怪物
    if FuncSet.GetGeneratedNpc(self._bossRefId) <= 0 then
        --防止断线重连时重复创建怪物
        self._bossId = FuncSet.GenerateNpc(self._bossRefId, 8004, ENpcCampType.Camp2, self._bossBornPos, self._bossBornRot)
    end
