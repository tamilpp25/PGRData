...
function XSObjElevator:HandleEvent(eventType, eventArgs)
    if eventType == EWorldEvent.SceneObjectTrigger then
        ...
        elseif eventArgs.SceneObjectId == self._sceneObjPlaceId and eventArgs.TriggerId == self._buttonPressTriggerId then
            if eventArgs.TriggerState == ESceneObjectTriggerState.Enter then
                if self._carriedNpcDict[eventArgs.SourceActorId] and self._buttonHasReleased then
                    self._buttonTriggerNpc = eventArgs.SourceActorId
                    self:StartMove()
                end
            end
        elseif eventArgs.SceneObjectId == self._sceneObjPlaceId and eventArgs.TriggerId == self._buttonReleaseTriggerId then
