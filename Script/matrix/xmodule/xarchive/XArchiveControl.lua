...
function XArchiveControl:GetAwarenessCollectRate()
    ...
    local haveNum = 0
    for _, _ in pairs(self._Model:GetAwarenessServerData()) do
        haveNum = haveNum + 1
    end
    return self:GetPercent(haveNum * 100 / sumNum)
end
...
function XArchiveControl:IsHaveNewAwarenessSuitByGetType(type)
    return self._Model:GetNewAwarenessSuitByGetType(type) > 0
end

function XArchiveControl:IsAwarenessSettingOpen(settingId)
    return self._Model:GetAwarenessSettingUnlockServerDataById(settingId) or self._Model:GetAwarenessSettingCanUnlockById(settingId) == true
end

-- 有红点的意识列表
function XArchiveControl:GetNewAwarenessSettingIdList(suitId)
    return self._Model:GetNewAwarenessSettingIdListById(suitId)
end

-- 意识的获得类型下是否有红点
function XArchiveControl:IsHaveNewAwarenessSettingByGetType(type)
    return self._Model:GetNewAwarenessSettingByGetType(type) > 0
end
...
function XArchiveControl:HandleCanUnlockAwarenessSettingByGetType(type)
    ...
    if groupDataList then
        ...
        for _, groupData in ipairs(groupDataList) do
            newSettingIdList = self._Model:GetNewAwarenessSettingIdListById(groupData.Id)
        ...
        XMVCA.XArchive:RequestUnlockAwarenessSetting(requestIdList)
    end
end
...
function XArchiveControl:HandleCanUnlockAwarenessSuit()
    ...
    if isHaveNew then
        ...
        XMVCA.XArchive:RequestUnlockAwarenessSuit(idList)
    end
end
...
function XArchiveControl:HandleCanUnlockAwarenessSuitByGetType(type)
    ...
    if isHaveNew then
        ...
        if groupDataList then
            ...
            XMVCA.XArchive:RequestUnlockAwarenessSuit(requestIdList)
        end
    end
end
...
function XArchiveControl:HandleCanUnlockAwarenessSetting()
    ...
    if isHaveNew then
        local idList = {}
        for id, _ in pairs(self._Model:GetAwarenessSettingCanUnlockDic()) do
            tableInsert(idList, id)
        end
        XMVCA.XArchive:RequestUnlockAwarenessSetting(idList)
    end
end
--endregion
...
function XArchiveControl:ClearCGRedPoint(id)
    if XSaveTool.GetData(self._Model:GetCGRedPointSaveKey(id)) then
        XSaveTool.RemoveData(self._Model:GetCGRedPointSaveKey(id))
    end
end
