...
function XCharacterAgency:GetFightCharHeadIcon(character, characterId) --获得战斗角色头像
    ...
    local isAchieveMaxLiberation = XDataCenter.ExhibitionManager.IsAchieveLiberation(characterId or character.Id, XEnumConst.CHARACTER.GrowUpLevel.Higher)
...
function XCharacterAgency:GetCaptainSkillInfo(characterId, skillLevel)
    ...
    if not skillLevel then
        local config = self._Model.SubSkillMinMaxLevelDicGrade[captianSkillId]
        if not config then
...
function XCharacterAgency:GetCharacterCareer(characterId)
    ...
    local detailConfig = self:GetCharDetailTemplate(characterId)
    if not detailConfig then
        return
    end

    local careerConfig = self:GetNpcTypeTemplate(detailConfig.Career)
...
function XCharacterAgency:GetEnhanceSkillLevelEffectBySkillIdAndLevel(skillId, level)
    if not self._Model:GetEnhanceSkillLevelEffectDic()[skillId] then
        XLog.Error("skillId Is Not Exist In ", skillId)
        return
    end
...
function XCharacterAgency:GetSkillGradeConfig(subSkillId, subSkillLevel)
    local skillLevelDict = self._Model.CharSkillLevelDict[subSkillId]
    ...
    return self:GetModelCharacterSkillUpgrade()[tabId]
end
...
function XCharacterAgency:ClampSubSkillLeveByLevel(skillId, skillLevel)
    ...
    if self._Model.SubSkillMinMaxLevelDicLevel[skillId].Max < fixSkillLevel then
        fixSkillLevel = self._Model.SubSkillMinMaxLevelDicLevel[skillId].Max
    elseif self._Model.SubSkillMinMaxLevelDicLevel[skillId].Min > fixSkillLevel then
        fixSkillLevel = self._Model.SubSkillMinMaxLevelDicLevel[skillId].Min
    end
...
function XCharacterAgency:ClampSubSkillLevelByGrade(skillId, skillLevel)
    local fixSkillLevel = skillLevel
    if self._Model.SubSkillMinMaxLevelDicGrade[skillId].Max < fixSkillLevel then
        fixSkillLevel = self._Model.SubSkillMinMaxLevelDicGrade[skillId].Max
    elseif self._Model.SubSkillMinMaxLevelDicGrade[skillId].Min > fixSkillLevel then
        fixSkillLevel = self._Model.SubSkillMinMaxLevelDicGrade[skillId].Min
    end
    return fixSkillLevel
end

function XCharacterAgency:GetSubSkillMinMaxLevel(subSkillId)
    return self._Model.SubSkillMinMaxLevelDicGrade[subSkillId]
end

function XCharacterAgency:GetSkillLevelEffectTemplate(skillId, level)
    local subSkills = self._Model.CharSkillLevelEffectDict[skillId]
    if (not subSkills) then
        XLog.Error("CharSkillLevelEffectDict not found skillId", skillId)
        return
    end

    local config = subSkills[level]
    if not config then
        XLog.Error("CharacterSkillLevelEffect.tab not found skillId, level", skillId, level)
        return
    end
...
function XCharacterAgency:GetCharacterGeneralSkillIds(characterId)
    local config = self._Model:GetCharacterDetail()[characterId]
    local res = string.ToIntArray(config.GeneralSkillIds, '|')
    return res
end
...
function XCharacterAgency:GetCharacterGeneralSkillRelatedSkillIds(characterId, generalSkillId)
    ...
    for _, skillGroupId in pairs(skillTemplate.SkillGroupId) do
        local skillIds = self:GetGroupSkillIdsByGroupId(skillGroupId)
        for _, skillId in pairs(skillIds) do
            local skillDescDetailConfig = self:GetSkillGradeDesWithDetailConfig(skillId, 1)
            local generalSkillIds = string.ToIntArray(skillDescDetailConfig.GeneralSkillIds, '|')
    ...
    if not XTool.IsTableEmpty(enhanceSkillTemplate) then
        for _, enhanceSkillGroupId in pairs(enhanceSkillTemplate.SkillGroupId) do
            ...
            for k, enhanceSkillId in pairs(skillIds) do
                local enhanceSkillDescConfig = self:GetEnhanceSkillGradeDescBySkillIdAndLevel(enhanceSkillId, 1)
                local generalSkillIds = string.ToIntArray(enhanceSkillDescConfig.GeneralSkillIds, '|')
                if table.contains(generalSkillIds, generalSkillId) then
