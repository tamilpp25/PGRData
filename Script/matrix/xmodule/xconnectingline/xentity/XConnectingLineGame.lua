...
function XConnectingLineGame:Execute(operation)
    ...
    if operation.Type == XEnumConst.CONNECTING_LINE.OPERATION_TYPE.POINT_DOWN then
        local grid = self:FindGrid(operation:GetPos())
        if not grid then
            return
        end
    ...
    if operation.Type == XEnumConst.CONNECTING_LINE.OPERATION_TYPE.POINT_MOVE then
        ...
        local grid = self:FindGrid(mousePos)
        -- 找不到格子
        if not grid then
            return
        end
        ...
        local possibleGrid = self._Map[possiblePos.X] and self._Map[possiblePos.X][possiblePos.Y]
