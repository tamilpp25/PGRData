...
----------public start----------
function XFSMAgency:CreateUpdatableFSM(controllerName, proxy, ...)
    if not controllerName or not proxy then
...
local MinusCountByLeavePlayState = function(self, xFSM)
    if self.AutoUpdateFSMDic[xFSM] == XEnumConst.FSM.AutoState.Play then
...
function XFSMAgency:SetAutoFSMPause(xFSM)
    ...
    MinusCountByLeavePlayState(self)
