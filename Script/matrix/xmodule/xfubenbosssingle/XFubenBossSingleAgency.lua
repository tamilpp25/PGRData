...
function XFubenBossSingleAgency:CheckAcitvityEnd(stageId)
    ...
    return stageInfo.Type == XEnumConst.FuBen.StageType.BossSingle and data:GetIsNeedReset()
end

function XFubenBossSingleAgency:CheckShowRecommend(featureId)
    return self._Model:GetBossSingleChallengeFeatureIsShowRecommendById(featureId)
end
...
function XFubenBossSingleAgency:OpenBossSingleView()
    if not self:IsBossSingleDataEmpty() then
        local levelType = self:GetBossSingleData():GetBossSingleLevelType()
        ...
        self:RequestSelfRank(Handler(self, self.OpenMainUi))
    end
end

function XFubenBossSingleAgency:OpenMainUi()
    if not self:IsBossSingleDataEmpty() then
        local data = self:GetBossSingleData()
...
function XFubenBossSingleAgency:CheckUnlockByStageId(stageId)
    ...
    return self:IsBossSingleTrial() or (stageInfo.Unlock or false)
end
