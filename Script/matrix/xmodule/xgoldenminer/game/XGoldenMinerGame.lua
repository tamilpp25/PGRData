...
function XGoldenMinerGame:GameSettle(isSkip)
    if not isSkip then
        self:GetGameData():SetTimeScore(self._MainControl:GetTimeScore(self:GetGameData():GetTime()))
        self:AddMapScore(self:GetGameData():GetTimeScore())
    end
...
function XGoldenMinerGame:_CheckIsStoneClear()
    ...
    for uid, _ in pairs(self:GetStoneEntityUidDirByType()) do
        ...
        if not self._GameClearIgnoreStoneTypeDir[stoneEntity.Data:GetType()] and
                not stoneEntity:CheckStatus(XEnumConst.GOLDEN_MINER.GAME_STONE_STATUS.GRABBED) and
                not stoneEntity:CheckStatus(XEnumConst.GOLDEN_MINER.GAME_STONE_STATUS.DESTROY) 
        then
            return false
        end
    end
...
function XGoldenMinerGame:GetStoneGoldIdByWeight(weight, id)
    ...
    if self:_GetCfgStoneType(id) == XEnumConst.GOLDEN_MINER.STONE_TYPE.GOLD then
        return
    end
    return self._CfgGoldWeightDic[weight]
end

function XGoldenMinerGame:_GetCfgStoneType(id)
