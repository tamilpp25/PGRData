...
function XGoldenMinerSystemBuff:_UpdateItemToEntity()
    ...
    if buff then
        for _, uid in ipairs(self._MainControl:GetHookEntityUidList()) do
            self._MainControl:GetHookEntityByUid(uid):GetComponentHook():SetAim(true)
        end
        self:TriggerCountBuff(buff)
    end
    ...
    if buff then
        for _, uid in ipairs(self._MainControl:GetHookEntityUidList()) do
            ...
            for _, stoneUid in ipairs(hookEntity:GetGrabbingStoneUidList()) do
                self._MainControl.SystemStone:SetStoneEntityStatus(self._MainControl:GetStoneEntityByUid(stoneUid), XEnumConst.GOLDEN_MINER.GAME_STONE_STATUS.BE_DESTROY)
            end
