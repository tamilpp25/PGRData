...
function XRogueSimControl:CheckCurStageIsTeach()
    ...
    return self:GetRogueSimStageType(stageId) == XEnumConst.RogueSim.StageType.Teach
end
...
function XRogueSimControl:GetHasPopupDataType()
    ...
    -- 区域解锁
    if self:CheckHasAreaUnlock() then
        return XEnumConst.RogueSim.PopupType.AreaUnlock
    end
...
function XRogueSimControl:ShowPopup(type, ...)
    ...
    elseif type == XEnumConst.RogueSim.PopupType.AreaUnlock then
        self:PlayAreaUnlock()
    else
...
-- 检查是否有区域解锁
function XRogueSimControl:CheckHasAreaUnlock()
    if not self.RogueSimScene then
        return false
    end
    return self.RogueSimScene:CheckHasAreaUnlock()
end

-- 播放区域解锁
function XRogueSimControl:PlayAreaUnlock()
    if not self.RogueSimScene then
        return
    end
    return self.RogueSimScene:PlayAreaUnlock()
end
...
-- 获取任务进度目标数量
---@param id number 配置Id
function XRogueSimControl:GetTaskTargetNum(id)
    local conditionId = self:GetTaskConditionId(id)
    local conditionParams = self:GetTaskConditionParams(id)
    if conditionId == 1 then
        return conditionParams[1] or 0
    elseif conditionId == 2 then
        return conditionParams[4] or 0
...
function XRogueSimControl:GetTaskScheduleAndTotalNum(id, configId)
    ...
    local totalNum = self:GetTaskTargetNum(configId)
...
function XRogueSimControl:OnCommodityChange()
    ...
    if self._Model.IsRoundEnd then
        XLog.Debug("XRogueSimControl IsRoundEnd is true")
        return
    end
    -- 是否需要请求
    ...
    for _, id in ipairs(XEnumConst.RogueSim.CommodityIds) do
        ...
        if sellCount > ownCount then
            sellPlan[id] = ownCount
            needRequest = true
        else
            sellPlan[id] = sellCount
        end
    end
    -- 有出售数量大于拥有数量的货物时，请求修改出售数量
    if needRequest then
        -- 移除value值为0的数据
        for k, v in pairs(sellPlan) do
            if v == 0 then
                sellPlan[k] = nil
            end
        end
        self:RogueSimCommoditySetupSellRequest(sellPlan)
    end
end
