...
function XRogueSimMapSubControl:GetEventSuspendDesc(id)
    local config = self._Model:GetRogueSimEventConfig(id)
    return config and config.SuspendDesc or ""
end
...
---@param eventId number 事件配置Id
function XRogueSimMapSubControl:CheckIsEventDuration(eventId)
    local duration = self:GetEventDuration(eventId)
    return duration > 0
end
...
function XRogueSimMapSubControl:CheckEventIsMustHandle(id)
    ...
    if not self:CheckIsEventDuration(configId) then
        return false
    end
...
function XRogueSimMapSubControl:CheckUnBuyBuildingsRedPoint()
    ...
    return self:CheckBuildingsRedPoint(ids)
end

-- 检查多个建筑红点
function XRogueSimMapSubControl:CheckBuildingsRedPoint(ids)
...
function XRogueSimMapSubControl:ExploreGrid(rewardData, cityId)
    if rewardData then
        if XTool.IsTableEmpty(rewardData.Rewards) then
            XLog.Error("XRogueSimMapSubControl:ExploreGrid Rewards is empty!")
