...
-- 检测贸易红点(库存紧张index = 3)
function XRogueSimResourceSubControl:CheckSellRedPoint()
    for _, id in ipairs(XEnumConst.RogueSim.CommodityIds) do
        if self:CheckCommodityIsWarning(id, 3) then
            return true
        end
    end
    return false
...
-- 检测生产红点(未选中生产货物、库存紧张index = 3)
function XRogueSimResourceSubControl:CheckProduceRedPoint()
    local id = self:GetProductCommodityId()
    if not XTool.IsNumberValid(id) then
        return true
    end
    return self:CheckCommodityIsWarning(id, 3)
end

-- 检测货物所处的警告状态
function XRogueSimResourceSubControl:CheckCommodityIsWarning(id, index)
    local count = self:GetCommodityActualCount(id)
    local limit = self:GetCommodityTotalLimit(id)
    -- 警告百分比
    local warningPercent = self:GetCommodityWarningPercent(id)
    return count >= limit * warningPercent[index] / 100
end
...
-- 先转换然后添加前缀和后缀
function XRogueSimResourceSubControl:ConvertNumberToIntegerWithPrefixAndSuffix(value, prefix, suffix)
    value = self:ConvertNumberToInteger(value)
    if not prefix then
        prefix = "+"
    end
    if not suffix then
...
---@param value number
---@return number
function XRogueSimResourceSubControl:ConvertNumberToInteger(value)
    value = getRoundingValue(value, 1)
