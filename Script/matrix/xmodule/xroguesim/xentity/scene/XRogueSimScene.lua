...
local SequenceIntervalTime = 0.2 -- 队列间隔时间
...
function XRogueSimScene:OnInit()
    ...
    self.AreaIdxUnlocks = self:GetAreaIndexUnlocks() -- 区域index是否解锁
...
function XRogueSimScene:PlayCacheExploreGrids(gridId, cb)
    ...
    local aimPos, time = self:GetCameraFocusGridPosAndTime(gridId)
    ...
    self.CameraSequence:AppendInterval(XEnumConst.RogueSim.MapCloudAnimTime + SequenceIntervalTime)
    ...
    -- 道具解锁视野
    if self:GetVisibleCacheType() == XEnumConst.RogueSim.AddVisibleGridIdType.ByArea then
...
function XRogueSimScene:CheckHasAreaUnlock()
    local unlocks = self:GetAreaIndexUnlocks()
    for idx, isUnlock in pairs(unlocks) do
        if not self.AreaIdxUnlocks[idx] and isUnlock then
            return true
        end         
    end
    return false
...
function XRogueSimScene:PlayAreaUnlock()
    ...
    for idx, isUnlock in pairs(unlocks) do
        if not self.AreaIdxUnlocks[idx] and isUnlock then
            table.insert(unlockAreaIds, areaIds[idx])
        end         
    end
    if #unlockAreaIds == 0 then
        return
    end
    self.AreaIdxUnlocks = unlocks
    ...
    self.CameraSequence:AppendInterval(XEnumConst.RogueSim.MapAreaOverlookTime + SequenceIntervalTime)

    -- 镜头移动
    local originPos = self.CameraFollowPointOverlook.localPosition
    for _, areaId in ipairs(unlockAreaIds) do
        ...
        self.CameraSequence:Append(self.CameraFollowPointOverlook:DOLocalMove(aimPos, time):SetEase(CsTween.Ease.OutQuad):OnComplete(function()
            self:OnAreaUnlock(tempAreaId)
        end))
        -- 区域云雾解锁+云雾移除
        self.CameraSequence:AppendInterval(XEnumConst.RogueSim.MapCloudAnimTime + XEnumConst.RogueSim.MapCloudAnimTime + SequenceIntervalTime)
        originPos = aimPos
    end
...
-- 获取区域的解锁列表
function XRogueSimScene:GetAreaIndexUnlocks()
    local unlocks = {}
...
function XRogueSimScene:OnAreaUnlock(areaId)
    ...
    for _, gridCfg in pairs(areaGridsCfg) do
        ...
        local unlockRender, removeRender = grid:SetAreaUnlock()
        if unlockRender then
            table.insert(unlockRenders, unlockRender)
        end
        if removeRender then
            table.insert(removeRenders, removeRender)
        end
    end

    -- 区域解锁表现
    self.RogueSimHelper:RevertAlteredCloudRenderers(unlockRenders, XEnumConst.RogueSim.MapCloudAnimTime, false)

    -- 移除可见格子的云
    self.CloudTimer = XScheduleManager.ScheduleOnce(function()
        self.CloudTimer = nil
        self.RogueSimHelper:DissolveCloudRenderers(removeRenders, XEnumConst.RogueSim.MapCloudAnimTime, false)
        for _, gridCfg in pairs(areaGridsCfg) do
            local grid = self.GridDic[gridCfg.Id]
            grid:Refresh()
        end
    end, (XEnumConst.RogueSim.MapCloudAnimTime + SequenceIntervalTime) * 1000)
end
...
function XRogueSimScene:OnLoadSceneComplete()
    if XEnumConst.RogueSim.IsDebug then
        XLog.Warning("当前区域为" .. XLog.Dump(self._MainControl.MapSubControl:GetAreaIds()))
    end
    ...
    self.UiCamera = sceneRoot:FindTransform("UiCamera"):GetComponent("Camera")
    ...
    self.CameraFollowPointOverlook = sceneRoot:FindTransform("CameraFollowPointOverlook"):GetComponent("RectTransform")

    self.CamNearOverlook = sceneRoot:FindTransform("CamNearOverlook")
    self.CamUiOverlook = sceneRoot:FindTransform("CamUiOverlook")
    ...
    self.GoInputHandler:AddPointerClickListener(function(eventData)
        ...
        if XEnumConst.RogueSim.IsDebug then
            XLog.Warning("点击位置 = " .. XLog.Dump(position))
        end
...
function XRogueSimScene:GenerateGridList()
    ...
    for i, areaId in ipairs(areaIds) do
        local isAreaUnlock = self.AreaIdxUnlocks[i] == true
        local areaGridsCfg = self._MainControl.MapSubControl:GetRogueSimAreaGridConfigs(areaId)
        for _, gridCfg in pairs(areaGridsCfg) do
...
function XRogueSimScene:ShowGridSelectEffect(gridId)
    ...
    for id, _ in pairs(visibleIdDic) do
        ...
        if vGrid:GetIsBlock() and vGrid:GetIsAreaUnlock() then
