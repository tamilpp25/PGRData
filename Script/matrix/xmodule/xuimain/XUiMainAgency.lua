...
function XUiMainAgency:GetScrollTipList(ignoreOnTerminal)
    ...
    for _, template in pairs(templates) do
        if not ignoreOnTerminal or (ignoreOnTerminal and template.IgnoreOnTerminal ~= 1) then
            ...
            local tip = func and func(self) or nil
...
function XUiMainAgency:GetExpensiveScrollTip()
    local allItem = self:GetAllActiveTipExpensiveiItems()
    if XTool.IsTableEmpty(allItem) or self:CheckPanelTipClicked(EnumConst.Ui_MAIN.TerminalTipType.ExpensiveItem) then
        return
    end
