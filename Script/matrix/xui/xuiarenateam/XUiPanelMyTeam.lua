local XUiPanelMyTeam = XClass(nil, "XUiPanelMyTeam")
...
function XUiPanelMyTeam:RegisterClickEvent(uiNode, func)
    if func == nil then
        XLog.Error("XUiPanelMyTeam:RegisterClickEvent函数参数错误：参数func不能为空")
        return
    end

    if type(func) ~= "function" then
        XLog.Error("XUiPanelMyTeam:RegisterClickEvent函数错误, 参数func需要是function类型, func的类型是" .. type(func))
    end
...
function XUiPanelMyTeam:OnBtnLeaveTeamClick()
...
function XUiPanelMyTeam:OnBtnJoinTeamClick()
    self.RootUi:JumpToHallPanel()
end

function XUiPanelMyTeam:OnBtnCreateTeamClick()
...
function XUiPanelMyTeam:Show()
    ...
    self.RootUi:PlayAnimation("MyTeamQieHuan")
