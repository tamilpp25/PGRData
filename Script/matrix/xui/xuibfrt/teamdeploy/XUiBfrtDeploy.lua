...
function XUiBfrtDeploy:UpdateQuickPassBtn()
    if not XDataCenter.BfrtManager.GetGroupClearOpen(self.GroupId) then
        self:_RefreshQuickPassBtn(false)
        return
    end
    local numberCount = 0
    ...
    if numberCount > 0 then
        local avgAbility = math.floor(totalAbility / numberCount)
        ...
        self:_RefreshQuickPassBtn(numberCount == requireNumberCount and avgAbility > XDataCenter.BfrtManager.GetGroupNeedAbility(self.GroupId))
    else
        self:_RefreshQuickPassBtn(false)
    end
end

function XUiBfrtDeploy:_RefreshQuickPassBtn(isShow)
    if self.BtnTongBlue then
        self.BtnTongBlue.gameObject:SetActiveEx(isShow)
    end
end
...
function XUiBfrtDeploy:OnBtnQuickClearClick()
    self:_SetTeam(function()
        ...
        XLuaUiManager.CloseWithCallback(self.Name, func)
    end)
end
