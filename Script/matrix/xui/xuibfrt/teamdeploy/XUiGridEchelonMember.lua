...
function XUiGridEchelonMember:GetProxyInstance(viewData)
    return {
        AOPCloseBefore = function(proxy, rootUi)
            XEventManager.DispatchEvent(XEventId.EVENT_BFRT_TEAM_UPDATE, rootUi.Team:GetEntityIds())
        end,
        --v2.6 新编队角色筛选器不用AOP
        ...
        AOPOnBtnJoinTeamClickedBefore = function(proxy, rootUi)
            ...
            if inEchelonIndex and inEchelonType then
                ...
                local finishCallback = function ()
                    XEventManager.DispatchEvent(XEventId.EVENT_BFRT_TEAM_SWAP, oldCharacterId, newCharacterId)
...
function XUiGridEchelonMember:OnBtnClickClick()
    ...
    local viewData = {
        ...
        TeamResultCb = function(team)
            XEventManager.DispatchEvent(XEventId.EVENT_BFRT_TEAM_UPDATE, team)
        end,
    }
