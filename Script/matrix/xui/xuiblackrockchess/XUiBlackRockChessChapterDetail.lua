...
function XUiBlackRockChessChapterDetail:UpdateView()
    ...
    self.TxtTarget.text = self._Control:GetStageTargetDesc(self._StageId)
        ...
        if cell.PanelArms then
            ...
            if data.isWeapon then
                ...
                self:RegisterClickEvent(cell.PanelArms, function()
                    XLuaUiManager.Open("UiBlackRockChessBubbleSkill", data.reward, cell.PanelArms, XEnumConst.BLACK_ROCK_CHESS.SKILL_TIP_TYPE.WEAPON)
                end)
            end
        end
        if cell.PanelSkill then
            ...
            if data.isPassSkill then
                ...
                self:RegisterClickEvent(cell.PanelSkill, function()
                    XLuaUiManager.Open("UiBlackRockChessBubbleSkill", data.reward, cell.PanelSkill, XEnumConst.BLACK_ROCK_CHESS.SKILL_TIP_TYPE.WEAPON_SKILL)
                end)
            elseif data.isBuff then
                ...
                self:RegisterClickEvent(cell.PanelSkill, function()
                    XLuaUiManager.Open("UiBlackRockChessBubbleSkill", data.reward, cell.PanelSkill, XEnumConst.BLACK_ROCK_CHESS.SKILL_TIP_TYPE.CHARACTER)
                end)
            end
...
function XUiBlackRockChessChapterDetail:ShowBuff()
    ...
    self:RefreshTemplateGrids(self.BtnAffix, buffIds, self.BtnAffix.parent, nil, "BlackRockChessDetailBuff", function(grid, data)
