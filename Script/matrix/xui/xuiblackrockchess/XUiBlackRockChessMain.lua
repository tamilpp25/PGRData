...
function XUiBlackRockChessMain:InitCb()
    ...
    self.BtnEasy.CallBack = function() 
        self:OnBtnChallengeClick(self._Control:GetFirstChapterId())
    end
    
    self.BtnHard.CallBack = function() 
        self:OnBtnChallengeClick(self._Control:GetSecondChapterId())
    end
...
function XUiBlackRockChessMain:UpdateView()
    ...
    local normalValue = self._Control:GetChapterProgress(self._Control:GetFirstChapterId()) * 100
    local hardValue = self._Control:GetChapterProgress(self._Control:GetSecondChapterId()) * 100
    ...
    self.BtnEasy:ShowReddot(self._Control:CheckHardRedPoint(self._Control:GetFirstChapterId()))
    self.BtnHard:ShowReddot(self._Control:CheckChapterTwoRedPoint() or self._Control:CheckHardRedPoint(self
            ._Control:GetSecondChapterId()))
...
function XUiBlackRockChessMain:CheckChapterEntrance()
    self._IsChapterOneOpen, self._ChapterOneConditionDesc = self._Control:IsNormalOpen(self._Control:GetFirstChapterId())
    self._IsChapterTwoOpen, self._ChapterTwoConditionDesc = self._Control:IsNormalOpen(self._Control:GetSecondChapterId())
...
function XUiBlackRockChessMain:OnBtnChallengeClick(chapterId)
    local isOne = chapterId == self._Control:GetFirstChapterId()
    local isTwo = chapterId == self._Control:GetSecondChapterId()
