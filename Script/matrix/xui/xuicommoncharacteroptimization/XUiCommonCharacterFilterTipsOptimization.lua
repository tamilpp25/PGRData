...
function XUiCommonCharacterFilterTipsOptimization:OnStart(characterList, cacheKeyName, filterCb, filterType, forceFilterFunction, groupNameList)
    ...
    if self.ForceFilterFunction then  -- 若有强制使用第三方筛选方法则使用          
        ...
        if resultFiltCharacterList and next(resultFiltCharacterList) then
            ...
            if self.FilterCb then
                self.FilterCb(resultFiltCharacterList)
            end
        else  -- 如果筛选结果为空,不处理
