...
function XUiGridFangKuaiBlock:UpdateBlock()
    ...
    self.PanelNum.gameObject:SetActiveEx(self.BlockData:GetBlockType() == XEnumConst.FangKuai.BlockType.BossHit)
    self.PanelNum.transform.localScale = self._ScaleVec3
    self.TxtNum.text = self.BlockData:GetMaxHitTimes() - self.BlockData:GetHitTimes()
...
function XUiGridFangKuaiBlock:PlayBossWane(isClear)
    -- 等整行消除特效播完再缩短长度
    local delayTime = isClear and 790 or 300
