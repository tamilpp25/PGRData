...
function XUiGachaLamiyaPassport:OnWear()
    ...
    if self._IsBackGround then
        ...
        XDataCenter.PhotographManager.ChangeDisplay(backgroundId, _curChara.Id, _curChara.FashionId, function()
            local state = XSaveTool.GetData(XDataCenter.PhotographManager.GetSceneStateKey(backgroundId))
            if state ~= 2 and XDataCenter.PhotographManager.CheckSceneIsHaveById(backgroundId) then
                state = 2
                XSaveTool.SaveData(XDataCenter.PhotographManager.GetSceneStateKey(backgroundId), state)
            end
