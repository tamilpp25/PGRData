...
function XUiGridKotodamaStage:Refresh()
    ...
    --是否解锁
    self.IsUnLock,self.StageIsInTime,self.PreIsPass=XMVCA.XKotodamaActivity:CheckStageIsUnLockById(self.Id)
...
function XUiGridKotodamaStage:OnClickEvent()
    if not self.IsUnLock then
        ...
        if not self.StageIsInTime then
            msg=XUiHelper.GetText('KotodamaStageNotInTime',XMVCA.XKotodamaActivity:GetStageUnLockLeftTime(self.Id))
        elseif not self.PreIsPass then
            msg=XUiHelper.GetText('KotodamaPreStageNotPassed')
        end
