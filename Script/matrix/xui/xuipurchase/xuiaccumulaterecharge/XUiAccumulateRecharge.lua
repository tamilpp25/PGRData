...
--region 按钮事件
function XUiAccumulateRecharge:OnBtnPCSwichClick()
...
function XUiAccumulateRecharge:OnPcSelectedIdChanged(selectId)
    self:_ShowCurrentRainbowCard(selectId)
end
...
function XUiAccumulateRecharge:_RegisterButtonClicks()
    ...
    self:RegisterClickEvent(self.BtnPCSwich, self.OnBtnPCSwichClick, true)
    self:RegisterClickEvent(self.BtnLook, self.OnBtnLookClick, true)
end

function XUiAccumulateRecharge:_ShowCurrentRainbowCard(selectId)
    if selectId == 8 then
        self.BtnPCSwich:SetName(XUiHelper.GetText("PCSwitchRainbowCard", "IOS"))
    elseif selectId == 10 then
        self.BtnPCSwich:SetName(XUiHelper.GetText("PCSwitchRainbowCard", "安卓"))
    end
end
...
function XUiAccumulateRecharge:_Init()
    if XDataCenter.UiPcManager.IsPc() then
        ...
        self:_ShowCurrentRainbowCard(id)
