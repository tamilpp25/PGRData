...
function XUiRiftBattleRoomProxy:AOPOnStartBefore(rootUi)
    ...
    rootUi.BtnTemplate.gameObject:SetActiveEx(XDataCenter.RiftManager.IsFuncUnlock(XRiftConfig.FuncUnlockId.Attribute))
...
function XUiRiftBattleRoomProxy:AOPRefreshRoleInfosAfter(rootUi)
    ...
    rootUi.BtnPanelLoad:GetComponent("XUiButton"):ShowReddot(XDataCenter.RiftManager:IsMemberAddPointRed())
    ...
    for index, roleId in pairs(team:GetEntityIds()) do
        ...
        if xRole then
            ...
            if txtLoad then
                txtLoad.text = CS.XTextManager.GetText(text, xRole:GetCurrentLoad(), XDataCenter.RiftManager.GetMaxLoad())
            end
            if txtFight then
                txtFight.text = xRole:GetFinalShowAbility()
            end
        end
    end
...
function XUiRiftBattleRoomProxy:RefreshTemplate()
    ...
    for index = 1, XRiftConfig.AttrCnt do
        ...
        uiObject.Normal.transform:Find("Txt0" .. index):GetComponent("Text").text = XRiftConfig.GetAttrName(index)
        pressUiObject[attrTxtName].text = tostring(attrLevel)
        uiObject.Press.transform:Find("Txt0" .. index):GetComponent("Text").text = XRiftConfig.GetAttrName(index)
    end
...
function XUiRiftBattleRoomProxy:OnBtnTemplateClick()
    XLuaUiManager.Open("UiRiftTemplate", XRiftConfig.DefaultAttrTemplateId, function(attrTemplateId)
        local attrTemplate = XDataCenter.RiftManager.GetAttrTemplate(attrTemplateId)
        local defaultAttrTemplate = XDataCenter.RiftManager.GetAttrTemplate(XRiftConfig.DefaultAttrTemplateId)
        defaultAttrTemplate:Copy(attrTemplate)
        XDataCenter.RiftManager.RequestSetAttrSet(defaultAttrTemplate, function()
            self:RefreshTemplate()
        end)
    end, nil, true, true)
end
