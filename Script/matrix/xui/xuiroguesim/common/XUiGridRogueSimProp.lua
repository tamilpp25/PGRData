...
function XUiGridRogueSimProp:GetPropId()
    return self.PropId
end
...
function XUiGridRogueSimProp:GetEffectDesc()
    ...
    -- 默认显示第一个buff的累计加成
    local cumulativeAdd = self._Control.BuffSubControl:GetBuffCumulativeAdd(buffIds[1], self.Id)
    -- 为空时不显示累计加成
    if not cumulativeAdd then
        return effectDesc
    end
    local addDesc = self._Control:GetClientConfig("PropBuffCumulativeMarkupTip")
    local cumulativeDesc = string.format(addDesc, cumulativeAdd)
    return string.format("%s%s", effectDesc, cumulativeDesc)
end

function XUiGridRogueSimProp:RefreshStatus()
    -- TODO 是否解锁
end
