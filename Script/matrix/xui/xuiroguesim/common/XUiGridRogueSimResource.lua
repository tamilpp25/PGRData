...
function XUiGridRogueSimResource:Refresh(id)
    ...
    if self.IsShowNum then
        ...
        if self.IsTurnSettle then
            ...
            local color = self._Control:GetClientConfig("RoundStarProduceColor", self.ProduceIsCritical and 2 or 1)
    ...
    if self.IsShowFluctuations then
        ...
        if percentage > 0 then
            self.TxtFluctuationsUp.text = self._Control.ResourceSubControl:ConvertNumberToIntegerWithPrefixAndSuffix(percentage)
        else
            self.TxtFluctuationsDown.text = self._Control.ResourceSubControl:ConvertNumberToIntegerWithSuffix(percentage)
        end
    end
end
