local XUiDlcSettleLose = XLuaUiManager.Register(XLuaUi, "UiDlcSettleLose")
...
function XUiDlcSettleLose:OnStart()
    ...
    self.TxtPeople.text = XUiHelper.GetText("BattleLoseActorNum", roomData:GetPlayerAmount())
...
function XUiDlcSettleLose:_Close()
    if self._WorldId then
        ...
        if agency then
            if not agency:ExCheckInTime() then
